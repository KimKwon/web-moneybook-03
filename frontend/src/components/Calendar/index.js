...
class Calendar extends Component {
  ...
  footerTemplate() {
    ...
    return `
        <span>
            <span>총 수입 ${Number(totalIncome).toLocaleString()}</span>
            <span>총 지출 ${Number(totalExpenditure * -1).toLocaleString()}</span>
        </span>
        <span>
            총계 ${Number(totalIncome - totalExpenditure).toLocaleString()}
        </span>
    `;
  }
