...
const getMonthlyData = async () => {
  const result = await findAll('account_history', {
    joinOptions: {
      table: [['payment_method', 'payment_method_id'], ['category', 'category_id'], ['users', 'user_id']],
    },
    where: {
      date: ['BETWEEN', ['2022-07-01','2022-07-31']],
    },
    fields: ['account_history.id', 'account_history.date', 'account_history.content', 'account_history.amount', 'payment_method.name as methodName', 'category.name as categoryName', 'category.id as categoryId']
  });
}
